...
const Products = () => {
  ...
  const handleScroll = (event, productIndex) => {
    ...
    setActiveIndices((prev) =>
      prev.map((index, i) => (i === productIndex ? newIndex : index)),
    );
  };
  ...
  return (
    <div className="px-4 md:px-[1rem] md:lg:px-[2rem] lg:px-[6rem] xl:px-[12rem]">
      <div className="pb-4">
        ...
        <p className="pb-2">
          These products you see are ready to ship. I want to keep my entire
          focus on the product quality, So I will setup a store after I make my
          first sale.
        </p>
        <p>
          <button
          ...
          Drop your contact info. I'll reach out instantly, and you'll make my
          day!
        </p>
      </div>
        ...
        <div key={productIndex} className="mb-12">
          ...
          >
            ...
            {product.images.map((img, imgIndex) => (
              <div
                key={imgIndex}
                className="w-full min-w-[300px] h-[250px] flex-shrink-0 overflow-hidden rounded-lg snap-center"
              >
                <img
                  src={`/products/${img}`}
            ...
            {product.images.map((_, index) => (
              <div
                ...
                className={`w-2 h-2 rounded-full mx-1 ${
                  index === activeIndices[productIndex]
                    ? "bg-[#845EC2]"
                    : "bg-[#DA95DE]"
                }`}
              />
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3 text-center">
              ...
              <div className="mt-4 px-7 py-3">
                <form
                  id="interestForm"
                  action="https://formspree.io/f/xvgpgqdo"
                  method="POST"
                >
                  <input
                    type="email"
                  ...
                  <button
                    type="submit"
                    className="mt-4 bg-[#845EC2] hover:bg-[#DA95DE] text-[#DFD7CF] hover:text-[black] hover:font-bold rounded px-4 py-2"
                  >
                    Submit
                  </button>
                </form>
                <button
                  onClick={closeModal}
                  className="mt-4 bg-gray-300 hover:bg-gray-400 text-black rounded px-4 py-2"
                >
                  Close
                </button>
              </div>
